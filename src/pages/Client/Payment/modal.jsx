const PromoCodeModal = ({ isOpen, onClose, promoCodes, onSelectPromoCode, totalAmount }) => {
                  ...
                  <div className="space-y-4">
                      {promoCodes.map((code, index) => (
                          code?.status === "Còn hạn" ? (
                              <div
                                  key={index}
                                  className={`border rounded-md p-4 flex items-center justify-between ${totalAmount < parseInt(code.gia_don_toi_thieu, 10) ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                                  onClick={() => totalAmount >= parseInt(code.gia_don_toi_thieu, 10) && onSelectPromoCode(code.ma_giam_gia)}
                                  style={{ cursor: totalAmount < parseInt(code.gia_don_toi_thieu, 10) ? 'not-allowed' : 'pointer' }}
                              >
